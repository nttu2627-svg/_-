...
def format_date_time(date_str):
    ...
    # 方法 1：改用冒號（完全 ASCII，最保險）
    output_format = '%Y年%m月%d日%H:%M'

    # 方法 2：改用繁體「點」
    # output_format = '%Y年%m月%d日%H點%M分'
...
def find_current_activity(current_time_str, schedule):
    """
    根據 current_time_str（'HH-MM'）回傳
    [activity, time_str]；若全部格式異常則回 ['睡觉', current_time_str]
    """
    current_time = datetime.strptime(current_time_str, '%H-%M')

    for item in schedule:
        # 只接受 [活動, 時間] 或 (活動, 時間)，忽略其他格式
        if not isinstance(item, (list, tuple)) or len(item) < 2:
            continue
        activity, time_str = item[0], str(item[1]).replace(":", "-")
